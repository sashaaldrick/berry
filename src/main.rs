...
enum Commands {
    ...
    /// Run the end-to-end tests with all prerequisites
    Run,
}
...
fn init_project(name: &str) -> Result<(), String> {
    ...
    println!("\nNext steps:");
    println!("1. cd {}", name);
    println!("2. berry setup && source env.sh");
    println!("3. export BONSAI_API_KEY=your_api_key_here  # Get one at https://bonsai.xyz/apply");
    println!("4. Run `anvil` in another terminal to start a local Ethereum node");
...
fn run_e2e() -> Result<(), String> {
    if !Path::new("e2e-test.sh").exists() {
        return Err(
            "e2e-test.sh not found. Please run this command from your project directory"
                .to_string(),
        );
    }

    // Check all environment variables from env.sh
    let required_vars = [
        (
            "BONSAI_API_URL",
            "Environment not set up. Please run 'source env.sh' first",
        ),
        (
            "ETH_WALLET_ADDRESS",
            "Environment not set up. Please run 'source env.sh' first",
    ...
    // Check for API key (this one is not in env.sh, needs to be set manually)
    if env::var("BONSAI_API_KEY").is_err() {
        return Err("BONSAI_API_KEY not set. Please set your API key".to_string());
    }

    println!("Starting anvil in background...");
    ...
    let status = Command::new("./e2e-test.sh")
        .status()
    ...
    anvil
        .kill()
        .map_err(|e| format!("Failed to stop anvil: {}", e))?;

    if !status.success() {
        return Err("Tests failed".to_string());
    }

    println!("{} Tests completed successfully", CHECK_MARK);
    Ok(())
}
